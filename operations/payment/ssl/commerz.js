...
const paymentFailedSSL = async (req, res) => {
  try {
    ...
    return res.redirect(
      `https://dwelling.emraan.me/payment/intent?txid=${txid}&pay=false&rm=${rm}&dwl=ling&ht=bright`
    );
  } catch (error) {
...
const paymentAcceptSSL = async (req, res) => {
  try {
    ...
    return res.redirect(
      `https://dwelling.emraan.me/payment/intent?txid=${txid}&pay=true&rm=${rm}&dwl=ling&ht=bright`
    );
  } catch (error) {
    return res.status(500).send({ msg: "Internal Server Error" });
  }
